-- Migration: Improve contractor to therapist synchronization
-- Description: Ensures all contractor profile data syncs properly to therapists table

-- Drop and recreate the sync function with improvements
...
-- Update existing contractor profiles to sync to therapists
-- This will trigger the sync function for all existing records
UPDATE contractor_profiles SET updated_at = NOW() WHERE user_id IS NOT NULL;

COMMENT ON FUNCTION sync_contractor_to_therapist IS 'Automatically syncs contractor profile data to therapists table with improved field mapping';
